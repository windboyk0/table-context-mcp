...
1. **Web UI 및 파싱 서버 (FastAPI)**
   ...
   - 업로드된 Excel 파일을 파싱하여 테이블별로 `JSON` 파일로 변환 후 로컬 디렉토리(`data/`)에 저장
...
└── data/                 # 파싱된 테이블별 고유 JSON 파일들이 저장될 디렉토리
```
...
- 서버는 pandas로 이를 파싱하여 테이블명(`테이블명` 필드 기준)을 추출하고 `data/{테이블명}.json` 파일로 각각 고유하게 저장
...
- 백엔드에서 LLM을 위해 호출될 Tool 함수들을 제공:
  - `list_tables()` : `data/` 폴더 내에 저장된 모든 테이블 목록 반환
  - `get_table_schema(table_name)` : 특정 테이블의 상세 컬럼 및 타입 정보 스키마 반환

## 다음 작업 (Task)
1. 사용자가 `sampleFile/TableDefinition_Sample.xlsx`에 샘플 엑셀 양식을 업로드하는 것 대기.
2. 엑셀 양식 구조 파악 후 `parser.py` 개발 시작.
3. `web_server.py` 및 UI 템플릿 제작.
4. `mcp_server.py` 개발 및 연결 테스트 수행.
